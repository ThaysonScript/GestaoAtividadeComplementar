...
public class SolicitacaoService {
	...
	public SolicitacaoValidacao anexarAtividade(Long solicitacaoId, Long atividadeId) {
		...
		if (solicitacao.getStatus() != StatusSolicitacao.SUBMETIDA && solicitacao.getStatus() != StatusSolicitacao.EM_ANALISE) {
			throw new SolicitacaoEmAbertoException("Nao e possivel anexar atividade a uma solicitacao que nao esta em aberto.");
		}
		...
		boolean jaExiste = solicitacao.getItens().stream()
				.anyMatch(i -> i.getAtividadeId().equals(atividadeId));
		if (jaExiste) {
