...
function handleSubstituicaoMocks(
  ...
  if (url.endsWith('/substituicao-comprovante') && method === 'POST') {
    return processarPostSubstituicao(req);
  }

  const regex = /\/substituicao-comprovante\/(\d+)$/;
  const idMatch = regex.exec(url);
  if (idMatch && method === 'PATCH') {
    return processarPatchSubstituicao(req, idMatch);
  }

  return null;
}

function processarPostSubstituicao(
  req: HttpRequest<unknown>,
): Observable<HttpResponse<unknown>> | null {
  const bodyObj = extrairDadosCorpo(req);
  ...
  const arquivoInvalido = validarArquivoSubstituicao(req);
  if (arquivoInvalido) {
    return arquivoInvalido;
  }

  return criarRespostaPostSubstituicao(req, atividade);
}

function validarArquivoSubstituicao(
  req: HttpRequest<unknown>,
): Observable<HttpResponse<unknown>> | null {
  if (!(req.body instanceof FormData)) return null;
...
function criarRespostaPostSubstituicao(
  req: HttpRequest<unknown>,
  atividade: {
    id: number;
    titulo: string;
...
function processarPatchSubstituicao(
  req: HttpRequest<unknown>,
  idMatch: RegExpExecArray,
): Observable<HttpResponse<unknown>> | null {
